defmodule BarcampWeb.Auth.UserControllerTest do
  ...
  describe "edit user" do
    ...
    test "renders form for editing chosen user", %{conn: conn, user: user} do
      conn = get conn, auth_user_path(conn, :edit, user)
  ...
  describe "update user" do
    ...
    test "redirects when data is valid", %{conn: conn, user: user} do
      conn = put conn, auth_user_path(conn, :update, user), user: @update_attrs
      assert redirected_to(conn) == auth_user_path(conn, :show, user)

      conn = get conn, auth_user_path(conn, :show, user)
      assert html_response(conn, 200) =~ "some updated email"
    end

    test "renders errors when data is invalid", %{conn: conn, user: user} do
      conn = put conn, auth_user_path(conn, :update, user), user: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit User"
    end
  end
  ...
  describe "delete user" do
    ...
    test "deletes chosen user", %{conn: conn, user: user} do
      conn = delete conn, auth_user_path(conn, :delete, user)
      assert redirected_to(conn) == auth_user_path(conn, :index)
      assert_error_sent 404, fn ->
        get conn, auth_user_path(conn, :show, user)
      end
    end
  end
